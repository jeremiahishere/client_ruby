...
module Prometheus
  module Client
    ...
    class Gauge < Metric
      ...
      def set(value, labels: {}, exemplar_labels: {})
        unless value.is_a?(Numeric)
          raise ArgumentError, 'value must be a number'
        end

        @store.set(labels: label_set_for(labels), val: value, exemplar_labels: exemplar_labels)
      end
      ...
      def increment(by: 1, labels: {}, exemplar_labels: {})
        label_set = label_set_for(labels)
        @store.increment(labels: label_set, by: by, exemplar_labels: exemplar_labels)
      end
      ...
      def decrement(by: 1, labels: {}, exemplar_labels: {})
        label_set = label_set_for(labels)
        @store.increment(labels: label_set, by: -by, exemplar_labels: exemplar_labels)
      end
    end
